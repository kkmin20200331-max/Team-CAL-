...
매장 운영자는 특정 시간대에 고객이 얼마나 몰리는지, 혼잡도가 언제 높아지는지 빠르게 파악해야 합니다.
...
단순 고객 수 추정에서 끝나지 않고, 시간대별 혼잡도 흐름을 다른 운영 데이터와 결합할 수 있는 AI 인사이트 API도 구현했습니다.
...
AI 서버가 단독으로 끝나지 않고 Spring Boot와 프론트엔드에서 시간대별 혼잡도 데이터를 활용할 수 있는 API 응답 구조를 설계했습니다.
...
- AI 인사이트 API에서 시간대별 혼잡도 흐름을 요약할 수 있도록 구조화했습니다.
