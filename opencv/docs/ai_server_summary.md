...
Python AI 서버는 CCTV 또는 영상 파일에서 사람 수를 추정하고, 시간대별 혼잡도 데이터를 만드는 분석 서버입니다.
...
이 구조 덕분에 30fps 원본 영상을 모든 프레임 단위로 처리하지 않아도 되고, 10초 단위 샘플링 기준으로 처리 대상 프레임을 약 99% 이상 줄일 수 있습니다. 최종적으로는 시간대별 사람 수와 혼잡도 흐름을 확인할 수 있는 집계 데이터를 생성합니다.
