    ...
    ({ date }: { date: Date }) => {
      ...
      return (
        <div className="flex flex-col items-center justify-center py-0.5">
          <span
            className={`text-sm leading-none ${isRed ? "text-red-500" : ""}`}
          >
            {date.getDate()}
          </span>
          {isHoliday && (
    ...
    switch (status) {
      ...
      case "pending":
        return (
          <Badge
            variant="outline"
            className="gap-1 border-yellow-400 text-yellow-600"
          >
          ...
          <TabsContent value="week" className="space-y-2 mt-4">
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <h3 className="font-bold text-sm">
                ...
                {format(addDays(currentWeekStart, 6), "M월 d일", {
                  locale: ko,
                })}
              </h3>
              <Button
            ...
            {loading ? (
              ...
              weekDates.map((date, index) => {
                ...
                return (
                  <Card
                    key={index}
                    className={isToday ? "border-purple-400 border-2" : ""}
                  >
                    <CardContent className="p-4">
                      <div className="flex items-start gap-4">
                        <div className="text-center min-w-[60px]">
                          <p
                            className={`text-xs ${index === 5 ? "text-blue-500" : index === 6 ? "text-red-500" : "text-gray-600 dark:text-gray-400"}`}
                          >
                            {format(date, "M/d")}
                          </p>
                          <p
                            className={`font-bold ${isToday ? "text-purple-600" : index === 5 ? "text-blue-500" : index === 6 ? "text-red-500" : ""}`}
                          >
                            {dayLabels[index]}
                            {isToday && (
