    ...
    <>
      ...
      >
        {profileImage
          ...
          : <span style={{ fontSize: 22, fontWeight: 800, color: '#fff' }}>{currentUser?.name?.[0] ?? <User />}</span>
        }
      </button>
      ...
      <div style={{
        ...
        background: panelBg, zIndex: 50, boxShadow: '-4px 0 32px rgba(0,0,0,0.24)',
        ...
        <div style={{
          ...
          <span style={{ fontWeight: 800, fontSize: 18, color: GREEN }}>{t.myProfile}</span>
          <button
            onClick={() => setOpen(false)}
            style={{
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '28px 28px', display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
            ...
            >
              <div style={{
                ...
                {profileImage
                  ? <img src={profileImage} alt="profile" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  : <span style={{ fontSize: 34, fontWeight: 800, color: '#fff' }}>{currentUser?.name?.[0] ?? '?'}</span>
                }
              </div>
              <div style={{
                position: 'absolute', bottom: 2, right: 2,
                width: 28, height: 28, borderRadius: '50%',
                background: DARK_GREEN, border: `2.5px solid ${panelBg}`,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
              }}>
          ...
          <div style={{ textAlign: 'center' }}>
            <p style={{ fontSize: 22, fontWeight: 800, color: textMain }}>{currentUser?.name || t.employee}</p>
            <span style={{
            ...
            }}>{currentUser?.role || t.employee}</span>
          </div>
          ...
          {storeName && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <MapPin style={{ width: 16, height: 16, color: '#5a8a5c', flexShrink: 0 }} />
        ...
        {/* 하단 */}
        <div style={{ padding: '16px 28px 28px', borderTop: `1px solid ${divider}`, display: 'flex', flexDirection: 'column', gap: 14 }}>
