...
const getStatusColor = (status: string) => {
  switch (status) {
    case 'confirmed': return 'bg-blue-100 border-blue-300 text-blue-900 dark:bg-blue-900/30 dark:border-blue-700 dark:text-blue-200';
    case 'pending':   return 'bg-yellow-100 border-yellow-300 text-yellow-900 dark:bg-yellow-900/30';
    case 'cancelled': return 'bg-red-100 border-red-300 border-dashed text-red-900 dark:bg-red-900/30';
...
export default function WeeklySchedule() {
  ...
  const fetchShifts = async () => {
    try {
      ...
      const res = await API.get('/shift', {
        params: { store_id: branchId, start_date: startDate, end_date: endDate }
      });
  ...
  const getShiftsForEmployee = (userId: string, date: Date): ShiftVO[] => {
    ...
    return shifts.filter(s => s.user_id === userId && getDateStr(s.work_date) === dateStr);
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-4">
              <Button variant="ghost" size="icon" onClick={() => navigate(`/admin/dashboard/${branchId}`)}>
                      ...
                      return (
                        <th
                          ...
                          className={`p-4 text-center border-b border-gray-200 dark:border-gray-700 min-w-[130px] cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 ${
                            holiday || isSunday ? 'bg-red-50 dark:bg-red-900/10' : isSaturday ? 'bg-blue-50 dark:bg-blue-900/10' : ''
                          }`}
                          onClick={() => navigate(`/admin/schedule/daily/${branchId}/${dateStr}`)}
                        >
                          <div className="flex flex-col items-center gap-0.5">
                            <span className={`text-sm font-medium ${holiday || isSunday ? 'text-red-500' : isSaturday ? 'text-blue-500' : ''}`}>
                              {format(date, 'EEE', { locale: ko })}
                            </span>
                            <span className={`text-lg font-bold ${holiday || isSunday ? 'text-red-600' : isSaturday ? 'text-blue-600' : ''}`}>
                              {format(date, 'd')}
                            </span>
                            {holiday && (
                              <span className="text-xs text-red-500 font-medium leading-tight">{holiday}</span>
                            )}
                          </div>
                        </th>
                      );
                          ...
                          return (
                            <td
                              ...
                              className={`p-2 border-b border-gray-200 dark:border-gray-700 text-center align-top ${
                                isHoliday || isSunday ? 'bg-red-50/30' : isSaturday ? 'bg-blue-50/30' : ''
                              }`}
                            >
