...
export default function Login() {
    ...
    const [language, setLanguage] = useState(() => localStorage.getItem('app-language') || 'ko');
    ...
    const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        setErrorMsg('');
        if (!username || !password) { setErrorMsg(t.errorEmpty); return; }
        try {
            const res = await loginAPI(username, password);
            localStorage.setItem('user', JSON.stringify(res.data));
            const { role } = res.data;
            if (role === 'ADMIN') navigate('/admin/branch-selection');
            else if (role === 'STAFF') navigate('/employee/home');
            else setErrorMsg(t.errorPending);
        } catch (err: any) {
            setErrorMsg(err.response?.status === 401 ? t.errorNoUser : t.errorServer);
        }
    };
            ...
            <div style={{ width: '100%', maxWidth: 480 }}>
                <div style={{
                    ...
                    <form onSubmit={handleLogin}>
                        ...
                        {errorMsg && (
                            <div style={{ color: '#e03434', fontSize: 13, textAlign: 'center', marginBottom: 12 }}>{errorMsg}</div>
                        )}
