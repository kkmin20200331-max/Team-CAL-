...
const getRealTimeItem = (item: Shift): Shift => { // ✅ [수정] ScheduleItem -> Shift
  ...
  let newStatus: Shift['status'] = 'COMPLETED'; // ✅ [수정] ScheduleItem -> Shift
...
const ScheduleScreen = ({ route }: { route: { params?: { userInfo: User | null } } }) => {
  ...
  const dummySchedule: Shift[] = [ // ✅ [수정] ScheduleItem -> Shift
  ...
  const [scheduleData, setScheduleData] = useState<Shift[]>(dummySchedule); // ✅ [수정] ScheduleItem -> Shift
  ...
  const [selectedShiftForLeave, setSelectedShiftForLeave] = useState<Shift | null>(null); // ✅ [수정] ScheduleItem -> Shift
  ...
  // 휴무 신청 버튼 클릭 시 모달 열기
  const handleOpenLeaveModal = (item: Shift) => { // ✅ [수정] ScheduleItem -> Shift
  ...
  // 근무 카드 컴포넌트
  const renderShiftCard = ({ item }: { item: Shift }) => { // ✅ [수정] ScheduleItem -> Shift
