...
const CHUNK_SIZE = 100; // 한 번에 번역할 텍스트 묶음 크기

/**
 * Google Cloud Translation API를 사용하여 텍스트를 번역합니다.
 * 텍스트가 많을 경우 자동으로 분할하여 여러 번 요청합니다.
...
export const translateTexts = async (texts: string[], targetLanguage: string): Promise<string[]> => {
  ...
  try {
    // 텍스트 배열을 CHUNK_SIZE에 맞춰 여러 묶음으로 분할
    for (let i = 0; i < texts.length; i += CHUNK_SIZE) {
      ...
      allTranslatedTexts = [...allTranslatedTexts, ...translations];
    }
